...
st.markdown("""
    ...
    .ad-node { background-color: #fffde7; border-left: 5px solid #ffd600; padding: 8px; margin-left: 15px; border-bottom: 1px solid #ddd; color: #b71c1c; }
    .pd-node { background-color: #f1f8e9; border-left: 5px solid #4caf50; padding: 6px; margin-left: 25px; color: #1b5e20; }
    </style>
...
with st.sidebar:
    ...
    loc_input = st.text_input("ಸ್ಥಳ ಹುಡುಕಿ", "Yellapur")
    if st.button("ಹುಡುಕಿ"):
        try:
    ...
    with t1:
        ...
        for p, d in pos.items():
            if mode == "Bhava" and v_div == 1:
                ...
                r_idx = get_varga_pos(d, v_div, lag_deg)
