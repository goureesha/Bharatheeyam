...
geolocator = Nominatim(user_agent="bharatheeyam_unified_fix_v2")
...
# !!! USING THE "DAYTIME PERFECT" LOGIC FOR SUNRISE/SET !!!
...
def get_full_calculations(jd, lat, lon):
    ...
    # ----------------------------------------------------
    # UNIFIED MANDI LOGIC 
    # (Uses Day-Perfect Engine + Night-Perfect Shifting)
    # ----------------------------------------------------
    ...
    # BRANCH 1: Pre-Sunrise (Vedic Night of Previous Day)
    if jd < sr_today:
        is_night = True
        ...
        panch_sr = prev_sr # For Panchanga display logic
        
    # BRANCH 2: Post-Sunset (Vedic Night of Current Day)
    elif jd >= ss_today:
        is_night = True
        w_idx = cal_wday
    ...
    # BRANCH 3: Daytime (Vedic Day of Current Day)
    else:
        is_night = False
        w_idx = cal_wday
        
        # Calculate Day Duration (Standard)
        start_base = sr_today
    ...
    # CONSTANTS
    # Day Factors [Sun...Sat]
    DAY_GHATI_ARR = [26, 22, 18, 14, 10, 6, 2]
    # Night Factors [Sun...Sat]
    NIGHT_GHATI_ARR = [10, 6, 2, 26, 22, 18, 14]
    ...
    pan = {
        ...
        "v": KN_VARA[w_idx], # Uses Vedic Weekday
        "n": KN_NAK[n_idx % 27],
