...
geolocator = Nominatim(user_agent="bharatheeyam_v98")
...
def get_full_calculations(jd, lat, lon):
    ...
    factor = night_ghati[vedic_wday] if is_night else day_ghati[vedic_wday]
    
    mtime = anchor + (duration * factor / 30.0)
    # Calculate Lagna degree for Mandi time
    m_res = swe.houses(mtime, float(lat), float(lon), b'P')
    positions[KN_PLANETS["Ma"]] = (m_res[1][0] - swe.get_ayanamsa(mtime)) % 360

    # 4. Panchanga Data
    pan = {
        "v": KN_VARA[vedic_wday],
        "udayadi": f"{int((jd - panchanga_sunrise)*60)} ಘಟಿ"
    }
...
if st.session_state.page == "input":
    with st.container():
        ...
        ampm = c3.selectbox("AM/PM", ["AM", "PM"], index=0)
    ...
    t1, t2, t3 = st.tabs(["ಕುಂಡಲಿ", "ಸ್ಫುಟ", "ಪಂಚಾಂಗ"])
    ...
    with t2:
        df_pos = pd.DataFrame([{"ಗ್ರಹ": k, "ರಾಶಿ": KN_RASHI[int(v/30)], "ಅಂಶ": f"{int(v%30)}° {int((v%30*60)%60)}'"} for k,v in pos.items()])
        st.table(df_pos)
