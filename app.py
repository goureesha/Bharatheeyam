...
# ==========================================
# 2. CORE MATH ENGINE (CRASH-PROOF)
# ==========================================
swe.set_ephe_path(None)
swe.set_sid_mode(swe.SIDM_LAHIRI)
geolocator = Nominatim(user_agent="bharatheeyam_mobile_v114")
...
def get_altitude(jd, lat, lon):
    res = swe.calc_ut(jd, swe.SUN, swe.FLG_EQUATORIAL | swe.FLG_SWIEPH)
    gmst = swe.sidtime(jd)
    lst = gmst + (lon / 15.0)
    ha = ((lst * 15.0) - res[0][0] + 360) % 360
    if ha > 180: ha -= 360
    lat_r, dec_r, ha_r = math.radians(lat), math.radians(res[0][1]), math.radians(ha)
...
def find_sunrise_set(jd_noon, lat, lon):
    ...
    for i in range(36):
        ...
        if get_altitude(t1, lat, lon) < -0.833 and get_altitude(t2, lat, lon) >= -0.833:
            l, r = t1, t2
            for _ in range(15):
                m = (l+r)/2
                if get_altitude(m, lat, lon) < -0.833: l = m
                else: r = m
            rise = r
        if get_altitude(t1, lat, lon) > -0.833 and get_altitude(t2, lat, lon) <= -0.833:
            l, r = t1, t2
...
def get_full_calculations(jd, lat, lon):
    ...
    night_f = [10, 6, 2, 26, 22, 18, 14]
    day_f = [26, 22, 18, 14, 10, 6, 2]
    f = night_f[w_idx % 7] if is_night else day_f[w_idx % 7]
...
if st.session_state.page == "input":
    with st.container():
        ...
        h, m, ampm = c1.number_input("ಗಂಟೆ", 1, 12, 2), c2.number_input("ನಿಮಿಷ", 0, 59, 43), c3.selectbox("M", ["AM", "PM"])
        lat, lon = st.number_input("Lat", value=14.98), st.number_input("Lon", value=74.73)
    ...
    with t1:
        ...
        v = c_v.selectbox("ವರ್ಗ", [1, 9, 3, 12, 30], format_func=lambda x: f"D{x}")
        b_opt = c_b.checkbox("ಭಾವ", value=False)
        bxs = {i: "" for i in range(12)}; ld = pos["ಲಗ್ನ"]
        for n, d in pos.items():
            if v == 1: ri = int(d/30) if not b_opt else (int(ld/30) + int(((d - ld + 360) % 360 + 15) / 30)) % 12
            else: ri = (([0,9,6,3][int(d/30)%4]) + int((d%30)/3.33333)) % 12 if v==9 else int(d/30)
    ...
    with t2:
        st.markdown("<div class='card'>", unsafe_allow_html=True)
        tbl_h = "<table class='key-val-table'><tr><th>ಗ್ರಹ</th><th>ರಾಶಿ</th><th>ಅಂಶ</th></tr>"
        for p, d in pos.items():
            tbl_h += f"<tr><td><b>{p}</b></td><td>{KN_RASHI[int(d/30)]}</td><td>{int(d%30)}° {int((d%30*60)%60)}'</td></tr>"
        st.markdown(tbl_h + "</table></div>", unsafe_allow_html=True)

    with t3:
        st.markdown(f"<div class='card' style='font-weight:bold;'>ಶಿಷ್ಟ ದಶೆ: {pan['d_bal']}</div>", unsafe_allow_html=True)
        dh = ""; cur_d = pan['date_obj']; si = pan['n_idx'] % 9
        for i in range(9):
            im = (si + i) % 9; md_dur = YEARS[im] * ((1 - pan['perc']) if i==0 else 1)
            me = cur_d + datetime.timedelta(days=md_dur*365.25)
            dh += f"<details><summary class='md-node'><span>{LORDS[im]}</span><span class='date-label'>{me.strftime('%d-%m-%y')}</span></summary>"
            cad = cur_d
            for j in range(9):
                ia = (im + j) % 9; ad_y = (YEARS[im] * YEARS[ia] / 120.0)
                if i == 0:
                    ad_y = ad_y * (1 - pan['perc'])
                ae = cad + datetime.timedelta(days=ad_y*365.25)
                dh += f"<div style='padding:8px 15px; border-bottom:1px solid #eee; display:flex; justify-content:space-between'><span>{LORDS[ia]}</span><span>{ae.strftime('%d-%m-%y')}</span></div>"
                cad = ae
            dh += "</details>"; cur_d = me
