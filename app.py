...
geolocator = Nominatim(user_agent="bharatheeyam_v108")
...
def get_full_calculations(jd, lat, lon):
    ...
    # Get Mandi's position (Lagna at mtime)
    ayan_m = swe.get_ayanamsa(mtime)
    m_res = swe.houses(mtime, float(lat), float(lon), b'P')
    positions[KN_PLANETS["Ma"]] = (m_res[1][0] - ayan_m) % 360
...
if st.session_state.page == "input":
    ...
    with t2:
        st.markdown("<div class='card'>", unsafe_allow_html=True)
        tbl = pd.DataFrame([{"ಗ್ರಹ": k, "ರಾಶಿ": KN_RASHI[int(v/30)], "ಅಂಶ": f"{int(v%30)}°"} for k,v in pos.items()])
        st.table(tbl); st.markdown("</div>", unsafe_allow_html=True)
    with t3:
        ...
        for i in range(9):
            ...
            cad = current_date
            for j in range(9):
                ia = (im + j) % 9; ad_years = (YEARS[im] * YEARS[ia] / 120.0); ae = cad + datetime.timedelta(days=ad_years*365.25)
                dh += f"<div style='padding:8px 15px; border-bottom:1px solid #eee; display:flex; justify-content:space-between'><span>{LORDS[ia]}</span><span>{ae.strftime('%d-%m-%y')}</span></div>"; cad = ae
