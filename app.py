...
# ==========================================
# 1. PAGE CONFIG & THEME (Restored 100%)
# ==========================================
...
geolocator = Nominatim(user_agent="bharatheeyam_godmode_v118")
...
def find_sunrise_set(jd_noon, lat, lon):
    ...
    for i in range(36):
        ...
        if get_altitude_manual(t1, lat, lon) < -0.833 and get_altitude_manual(t2, lat, lon) >= -0.833:
            l, r = t1, t2
            for _ in range(15):
                m = (l+r)/2
        ...
        for i in range(9):
            ...
            for j in range(9):
                ...
                for k in range(9):
                    ...
                    dh += f"<div class='pd-node' style='padding:8px 15px; border-bottom:1px solid #eee; display:flex; justify-content:space-between'><span>{LORDS[ip]}</span><span>{pe.strftime('%d-%m-%y')}</span></div>"
                    cpd = pe
                dh += "</details>"; cad = ae
